...
public class VectorSearchService {
    ...
    private final ConcurrentHashMap<String, ConcurrentHashMap<String, Map<String, Double>>> vectorsByNamespace = new ConcurrentHashMap<>();

    public void upsert(String namespace, String text, String cacheKey) {
        vectorsByNamespace
                .computeIfAbsent(namespace, ignore -> new ConcurrentHashMap<>())
                .put(cacheKey, embed(text));
    }

    public Optional<String> search(String namespace, String text, double threshold) {
        Map<String, Map<String, Double>> vectorsByCacheKey = vectorsByNamespace.get(namespace);
        if (vectorsByCacheKey == null || vectorsByCacheKey.isEmpty()) {
    ...
    public void remove(String namespace, String cacheKey) {
        Map<String, Map<String, Double>> vectorsByCacheKey = vectorsByNamespace.get(namespace);
        if (vectorsByCacheKey != null) {
            vectorsByCacheKey.remove(cacheKey);
        }
    }

    public int clear() {
        int size = vectorsByNamespace.values().stream().mapToInt(Map::size).sum();
