(function () {
    ...
    const newConnName = document.getElementById("newConnName");
    const newConnUrl = document.getElementById("newConnUrl");
    const newConnUser = document.getElementById("newConnUser");
    const newConnPassword = document.getElementById("newConnPassword");
    const newConnDriver = document.getElementById("newConnDriver");
    ...
    viewSchemaBtn.addEventListener("click", () => loadSchemaPreview(activeConnectionId));
    ...
    connectionSelect.addEventListener("change", async () => {
        ...
        try {
            const activated = await fetchJson(`/api/connections/${encodeURIComponent(selectedId)}/activate`, {method: "POST"});
            activeConnectionId = activated.id;
            setStatus(`Active DB: ${activated.name}`);
            await loadSchemaPreview(activeConnectionId);
            showToast(`Switched to ${activated.name}`, "info");
        } catch (error) {
            showToast(error.message, "error");
    ...
    newConnectionForm.addEventListener("submit", async (event) => {
        event.preventDefault();
        const name = (newConnName.value || "").trim();
        const jdbcUrl = (newConnUrl.value || "").trim();
        if (!name || !jdbcUrl) {
            showToast("Connection name and JDBC URL are required", "error");
            return;
        }
        try {
            await fetchJson("/api/connections", {
                ...
                body: JSON.stringify({
                    name,
                    jdbcUrl,
                    username: newConnUser.value || "",
                    password: newConnPassword.value || "",
                    driverClassName: newConnDriver.value || ""
                })
            });
    ...
    async function askQuestion(question) {
        const connectionUsed = activeConnectionId || "local";
        appendTextBubble("user", question, "You");
        setStatus(`Thinking (${connectionUsed})...`);
        setAskEnabled(false);
        ...
        try {
            const answer = await fetchJson("/api/chat/ask", {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({question, userId, connectionId: connectionUsed})
            });
            loadingNode.remove();
            appendAssistantBubble(answer, question, connectionUsed);
    ...
    function appendAssistantBubble(answer, originalQuestion, connectionUsed) {
        const bubble = document.createElement("div");
        bubble.className = "bubble assistant";
        ...
        meta.appendChild(buildPill(`DB: ${connectionUsed}`));
    ...
    function renderHistoryFromFilter() {
        ...
        filtered.forEach((item) => {
            ...
            meta.textContent = `${item.connectionId || "local"} | ${item.source} | ${item.rowCount} rows | ${item.latencyMs} ms`;
    ...
    async function loadConnections() {
        try {
            ...
            if (!activeConnectionId && connectionSelect.options.length > 0) {
                activeConnectionId = connectionSelect.options[0].value;
            }
            await loadSchemaPreview(activeConnectionId);
        } catch (error) {
            schemaPreview.textContent = "Unable to load connections.";
            showToast(error.message, "error");
        }
    }

    async function loadSchemaPreview(connectionId) {
        if (!connectionId) {
            schemaPreview.textContent = "No active connection.";
            return;
        }
        try {
            const payload = await fetchJson(`/api/connections/schema/prompt?connectionId=${encodeURIComponent(connectionId)}&maxTables=15&maxColumnsPerTable=12`);
            schemaPreview.textContent = payload.schemaPrompt || "Schema unavailable.";
        } catch (error) {
            schemaPreview.textContent = "Unable to load schema preview.";
        }
    }
